...
def home():
    ...
    foto_perfil = url_for('static', filename='fotos_perfil/')
    return render_template("home.html", posts = posts, foto_perfil=foto_perfil)
...
def usuarios():
    ...
    foto_perfil = url_for('static', filename='fotos_perfil/')
    return render_template("usuarios.html", lista_usuarios = lista_usuarios, foto_perfil = foto_perfil)
...
def login():
    ...
    if form_login.validate_on_submit() and "botao_submit_login" in request.form:
        ...
        if usuario and bcrypt.check_password_hash(usuario.senha, form_login.senha.data):
            ...
            flash(f"Login feito com sucesso no email {form_login.email.data}", category='alert-success')
    ...
    if form_criarconta.validate_on_submit() and "botao_submit_criarconta" in request.form:
        ...
        flash(f"Conta criada com email {form_login.email.data}", category='alert-success')
        return redirect(url_for("home"))
...
def sair():
    logout_user()
    flash(f"Logout realizado com sucesso", category='alert-success')
    return redirect(url_for('home'))
...
def editar_perfil():
    ...
    if form_editarperfil.validate_on_submit():
        ...
        database.session.commit()
        flash ("Dados alterados com sucesso", category='alert-success')
        return redirect(url_for('perfil'))
    elif request.method == 'GET':
        form_editarperfil.email.data = current_user.email
        form_editarperfil.username.data = current_user.username
    foto_perfil = url_for('static', filename='fotos_perfil/{}'.format(current_user.foto_perfil))
    return render_template("editarperfil.html", foto_perfil = foto_perfil, form = form_editarperfil)
...
def exibir_post(post_id):
    ...
    if current_user == post.autor:
        ...
        form = None
    return render_template("post.html", post=post, foto_perfil=foto_perfil, form=form)
...
def excluir_post(post_id):
    ...
    if current_user == post.autor:
        ...
        flash("Comando não autorizado", "alert-danger")
        return redirect(url_for("home"))
